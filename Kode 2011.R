...
df <- subset(df_growth2019, select = c("avg_gdpgrowth", "avg_p", "avg_nsy", "avg_educ", "avg_nry", "avg_gi","avg_n","avg_gx", "ln_gdppc0"))
# Gi beskrivende navn til variablene (i samme rekkefølge som de ligger i datasettet)
labs <- c("Gjennomsnitlig årlig vekstrate i BNP pc 2000-2019 (%)", "Gjennomsnittlig årlig befolkningsvekst (%)", "Gjennomsnittlig netto sparing", "Gjennomsnittlig vekstrate i investeringer", "Gjennomsnittlig antall år i skole", "Gjennomsnittlig årlig reduksjonsrate i naturressurser", " årlig vekstrate i eksport", "BNP per innbygger i år 2000", "Vekst i Arbeidskraft" ) 
# Lag tabellen
st(df, labels=labs,
   title = "Tabell 1. Deskriptiv statistikk",
   summ = list(
...
plot1 <- ggplot(df_growth2019_n, aes(x = avg_p , y = ln_gdppc, na.rm = TRUE)) +
  ggtitle("Plot 1,Befolkningsvekst og BNP per inbygger 2019")+
...
plot2 <- ggplot(df_growth2019_n, aes(x = avg_nsy , y = ln_gdppc, na.rm = TRUE)) +
  ggtitle("Plot 2,sparing og BNP per inbygger 2019")+
...
plot3 <- ggplot(df_growth2019_n, aes(x = avg_educ , y = ln_gdppc, na.rm = TRUE)) +
  ggtitle("Plot 3,Humankapital og BNP per inbygger 2019")+
...
plot4 <- ggplot(df_growth2019_n, aes(x =avg_nsy  , y =avg_gdpgrowth , na.rm = TRUE)) +
  ggtitle("Plot 4,nettosparing og vekst i bnp per inbygger")+
  xlab("nettosparing") + # Beskrivelse for x-akselen
  ylab("vekst i bnp") + # Beskrivelse for y-akselen
  theme_minimal(base_size = 14, base_family = "Georgia") + # Tekststørrelse og font
  geom_point(aes(size = poptot, color = region), alpha = 0.8) + # Størrelse (farge) på bobblene avhenger befolkningsstørrelse (region)
  scale_x_continuous(labels = dollar)  + # Legg til et dollar tegn til y-akselen
  scale_size_area(guide = "none", max_size = 14) + #Ta vekk legend for befolkningsstørrelse
...
plot5 <- ggplot(df_growth2019_n, aes(x = avg_educ , y = avg_gdpgrowth, na.rm = TRUE)) +
  ggtitle("Plot 5,Humankapital og vekst i bnp per inbygger")+
  xlab("Humankapital") + # Beskrivelse for x-akselen
...
model1<- lm(avg_gdpgrowth  ~ avg_educ+ avg_p+ avg_nsy+ avg_nry+ avg_gi+ avg_n+ avg_gx+ ln_gdppc0,data= df_growth2019)
tab_model(model1)
...
model2 <- lm(avg_gdpgrowth  ~ avg_educ+ avg_p+ avg_nsy+ avg_nry+ avg_gi+ avg_n+ avg_gx+ ln_gdppc0,data= no_outliers)
tab_model(model2)
